...
public class NormalRandomItem extends Activatable
{
	...
	private static NormalRandomItem instance;
	
	private NormalRandomItem()
	{
		super("random_item");
	}
	...
	public void run(Player p)
	{
		...
		{
			nextItem = ItemSpecial.getInstance(ContainmentPickaxe.class).getItemStack(null, (float) (r.nextInt(25) + r.nextDouble()));
			setCommodity = false;
		}
		else if(nextDouble < (d += 1.0D))
		{
			nextItem = ItemSpecial.getInstance(LegendaryRandomItem.class).getItemStack(null, null, version);
			setCommodity = false;
		}
		else if(nextDouble < (d += 4.0D))
		{
			nextItem = ItemSpecial.getInstance(RareRandomItem.class).getItemStack(null, null, version);
			setCommodity = false;
		}
		else if(nextDouble < (d += 15.0D))
		{
			nextItem = ItemSpecial.getInstance(CommonRandomItem.class).getItemStack(null, null, version);
			setCommodity = false;
		}
